...
─────────────────
  ...
  4. Forward confirmed gesture events to Unity + Monitor over UDP.
...
─────────────────────────────────────────
  ...
  GestureEngine.predict(frame_buffer)
        ...
        └──► UDP  5005 (Unity)  +  5006 (Monitor)   [only if != 'none']
"""
...
app = Flask(__name__)
app.config["SECRET_KEY"] = "antiradar_v6_secret"
CORS(app)
socketio = SocketIO(
    app,
...
def _udp_send(gesture: str) -> None:
    """
    Send a gesture string to both Unity (5005) and the local monitor (5006).
...
def _radar_loop() -> None:
    ...
    One iteration = one radar frame:
      ...
      6. if gesture != 'none': UDP to Unity + Monitor
    """
...
if __name__ == "__main__":
    ...
    if os.environ.get("WERKZEUG_RUN_MAIN") != "true":
        print("=" * 65)
        print("  RADAR GESTURE RECOGNITION  —  V6  Central Hub")
        print(f"  Dashboard  →  {url}")
        print(f"  Radar Pi   →  {PI_HOST}:{PI_PORT}")
        print(f"  Unity UDP  →  {UNITY_IP}:{UNITY_PORT}")
        print(f"  Monitor    →  {UNITY_IP}:{MONITOR_PORT}")
