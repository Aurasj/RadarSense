...
# Keep radar running during dashboard refresh/disconnect by default.
# Set AUTO_STOP_ON_NO_CLIENTS=1 if you want old behavior.
AUTO_STOP_ON_NO_CLIENTS = os.environ.get("AUTO_STOP_ON_NO_CLIENTS", "0") == "1"

# Reduce browser/socket load: server keeps full raw frame for ML,
# dashboard receives a lightweight 128-bin visual frame.
...
def _resample_1d(x: np.ndarray, out_bins: int) -> np.ndarray:
    """
    Downsample/interpolate one radar envelope frame for the dashboard.
    This does NOT affect ML inference; GestureEngine still receives raw frames.
    """
...
def on_disconnect(reason=None):
    ...
    if _connected_clients == 0 and AUTO_STOP_ON_NO_CLIENTS:
        _stop_radar()
